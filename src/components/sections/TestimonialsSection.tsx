        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((testimonial) => (
            <div 
              key={testimonial.id} 
              id={testimonial.id}
              className="bg-gray-800 rounded-lg p-8 relative transition-all duration-300 hover:transform hover:-translate-y-2 hover:shadow-lg"
            >
              <div className="mb-6">
                <svg className="w-10 h-10 text-tech-blue opacity-30" fill="currentColor" viewBox="0 0 24 24">
              ...
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-tech-blue to-gold transform scale-x-0 origin-right transition-transform duration-300 group-hover:scale-x-100"></div>
            </div>
          ))}
        </div>
